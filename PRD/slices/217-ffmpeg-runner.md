# Slice #217 — Secure ffmpeg runner and behavioral probe
...
* S217-R1: construct a fixed argv vector from validated executable/profile
...
* S217-R2: each child shall have one rawvideo input and one NUT output.
  Intended endpoints survive exec; all others are close-on-exec. Parent ends
  are nonblocking and owned by one handle.
* S217-R3: input shall be committed only after the complete view is available.
...
* S217-R4: request at least 64 KiB pipe capacity and verify the effective size.
...
* S217-R5: parse output with #214, validate it with #215, enforce one packet
  per submitted view and bounded total bytes, and associate by FIFO submission
  identity. Extra, missing or late packets fail.
* S217-R6: before capability confirmation, run a bounded behavioral probe at
...
* S217-R7: the ffmpeg input command shall include an explicit one-frame
  `probesize` so small geometry cannot wait for additional frames.
* S217-R8: close/recycle shall terminate, reap and release every descriptor
  and parser buffer. Partial startup shall use the same cleanup path.
* S217-R9: `dump_extra` shall statically select whether ffmpeg reinserts global
...
* S217-R10: the runner shall emit `feedend` with `sequence`, `main` and

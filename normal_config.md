...
> the **regression control** — the stock behaviour any custom-built dev `.deb`
> is compared against — see **`build_config.md`** (build, package, deploy,
> headless checks) and **`dev_config.md`** (the interactive RDP test). Both assume
> this baseline is already in place — in particular `xorgxrdp`, a separate Xorg
> module **not** built from this tree (and which must be version-matched — see
> `build_config.md` Part X).
>
...
# Gate 9 — Capture the stock baseline

Once a session is up, record the stock behaviour so any custom-built dev `.deb`
can be compared against it. Run inside the RDP session:

```bash
xrandr                                # outputs and resolution
xdpyinfo | grep -E 'dimensions|resolution'
sudo grep -E 'Starting X server' \
    /var/log/xrdp.log /var/log/xrdp-sesman.log | tail
```

Save this output as the regression control: after deploying a dev `.deb`, a
normal (~96-DPI) client must still produce this same baseline, unchanged.

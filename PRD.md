...
2. *frame_num slots.* Shared-counter rewrite over a synthetic `M A M A …` interleave: golden per-AU frame_num sequence; wrap vectors crossing 255→0 (`log2_max_frame_num=8`) for both views; guard vectors for aux cadences ≠ 1:1 (sparse aux must not desynchronize the counter).
...
4. *Negative/fail-loud vectors.* Aux slice shapes outside the guard (unexpected slice_type, existing mmco chain, list modification already present, `max_num_ref_frames` too small for the level) must hard-reject, never silently emit.

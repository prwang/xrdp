...
**Invariance contract (RELAXED relative to FR-H264-7, matching Windows).** Required: (a) main-chain independence — decode with all aux AUs dropped bit-identical (Windows measured 348/348; `tools/avc444_topology_check.sh` topology 2); (b) deterministic reference resolution in the single-decoder interleaved feed. NOT claimed: FR-H264-7 topology 3 (aux-only per-view decode) — the Windows stream itself fails it (first-aux→LT0; frame_num gaps under `gaps_allowed=0`) yet renders on every client. An FR-H264-8 capture is therefore expected-RED on topology 3; the checker run must record that as the documented expectation, not silently skip it.

**Implementation sketch.** Same two children as FR-H264-7; the aux child encodes a normal `refs=1` P chain instead of all-IDR. The splicer rewrites both views’ slice headers (pre-CABAC, existing machinery): shared frame_num counter, constant mmco6 self-mark, constant LTR list-modification; both children’s CABAC payloads stay byte-verbatim (each child’s `ref_idx 0` remaps to its LT slot via the modification list). SPS splice: raise `max_num_ref_frames`, re-check level DPB limits. Fail-loud on any slice shape outside the guard, as today.

**Acceptance gate (to leave EXPERIMENTAL).** (1) Offline synthesized stream decodes bit-identical (framemd5) to each child’s own decode; (2) drop-aux bit-identity on a live capture; (3) fleet arm (VAAPI) + T4 (nvenc) wire captures verified, with the topology-3 expected-RED recorded; (4) macOS onscreen verdict by owner; (5) bitrate/perf record vs the leaf arm on identical content; (6) owner sign-off recorded in BACKLOG before any default change.

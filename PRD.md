...
`encode_single()` is already **submit-then-collect** internally (it pushes to the vmsplice iov queue, then blocks in `pump()`), so the call sites split cleanly — but **the split alone buys nothing** (corrected 2026-07-28 while specifying #45). `in_iov_push()` performs no I/O: every byte of transfer, and every byte of output drain, happens inside `pump()`, which polls ONE child's three fds. "Submit both, then collect both" therefore still serialises the two encodes. Real overlap needs a **union-poll pump across both children** under one shared deadline. Collecting main to completion before servicing aux's stdout is also unsafe: a 4K intra packet exceeds a default pipe buffer, so an undrained main (or aux) child blocks on write and stalls the pair it was supposed to overlap.

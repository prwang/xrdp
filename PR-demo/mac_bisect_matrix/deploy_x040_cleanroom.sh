#!/bin/bash
# Build and deploy the paired clean-room candidate on the next local endpoint.
set -eu

D=$(cd "$(dirname "$0")" && pwd)
DIST=${DIST:-/work/dist}
XRDP_DEB=${XRDP_DEB:-xrdp-dev_0.10.80+git20260825041007.ae2a4674ab96_amd64.deb}
XORGXRDP_DEB=${XORGXRDP_DEB:-xorgxrdp-dev_1%3a0.10.80+git20260825030213.aa03d860137d_amd64.deb}
IMAGE=${IMAGE:-localhost/xrdp-bisect:cleanroom-ae2a4674-aa03d860-u2404-xfce.p736e0983}
BUILD="$D/.build-x040-cleanroom"
...
kubectl -n bisect-matrix create configmap xrdp-gfx-x040 \
    --from-file=gfx.toml="$D/gfx/x040.toml" \
    --dry-run=client -o yaml | kubectl apply -f -
kubectl apply -f "$D/k8s/x040.yaml"
kubectl -n bisect-matrix rollout status deployment/xrdp-x040 --timeout=300s

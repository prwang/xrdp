/* mstsc-style AVC444 v2 reconstruction test.
 * Same Luma + ChromaV2 combine + reverse-filter RGB as the FreeRDP-faithful
 * decoder, EXCEPT the ChromaV2 U|V split width (nTotalWidth) is overridable:
 *   argv[6] = nTotalWidth used for the split (default = coded 16-aligned width)
 * The aux plane STRIDE stays at the real coded width (cw) — mstsc still decodes
 * a cw-wide H.264 frame; only its assumed split alignment differs.
 *   nT = round_up_16(W)  -> our/FreeRDP packing (clean)
 *   nT = round_up_32(W)  -> hypothesised mstsc split (burr on odd MB count)
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int main(int c,char**v){int W=atoi(v[3]),H=atoi(v[4]),ch=(H+15)&~15;
 ...
 Luma(mY,mU,mV,cw,chw,chw,dY,dU,dV,cw,W,H);R16 r={0,0,W,H};ChV2(aY,aU,aV,cw,chw,chw,(UINT32)nT,dU,dV,cw,r);
 BYTE*rgb=calloc((size_t)W*H*3,1);RGB(dY,dU,dV,cw,W,H,rgb);
 FILE*o=fopen(v[5],"wb");fwrite(rgb,1,(size_t)W*H*3,o);fclose(o);
 fprintf(stderr,"decoded %dx%d cw=%d nT=%d (round_up_32=%d) -> %s\n",W,H,cw,nT,(W+31)&~31,v[5]);
 return 0;}

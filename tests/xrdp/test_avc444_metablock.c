 ...
 *
 * Region-strict H.264 decoders reconstruct chroma one region rect at a time
 * and index the odd chroma columns/rows relative to the rect origin. An odd
 * rect left/top therefore flips chroma parity and fringes the rect's left/top
 * edge (a chroma fringe on high-contrast vertical/horizontal edges of every

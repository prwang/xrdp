...
START_TEST(test_tconfig_gfx_avc444_defaults)
{
     ...
     * window of 2 this line asserted from 2026-08-07). Both halves are
     * asserted here because they are a pair -- the frontier at a window
     * of 1 is behaviourally the legacy gate, so the mechanism alone does
     * not describe what ships, and the pair is what makes "an upgrade
     * that changes no configuration keeps today's wire behaviour" a
     * checkable claim. Expected values are the directive, not a reading
     * of the loader.
     *
    ...
    ck_assert_int_eq(gfxconfig.avc444_ffmpeg_wire_window, 1);
}
END_TEST

 ...
 *
 * MS-RDPEGFX AVC capability classification (PRD FR-CAP-1 / FR-CAP-2).
 *
 * Pure input->output logic mapping (capability version + flags) to the
 * AVC wire mode this MVP would select for that single advertised capset.
 * The MVP external ffmpeg backend supplies AVC444 v1 only; AVC420 and
 ...
 *
 * Returns the AVC mode candidate this capset supports per FR-CAP-2:
 *   v8            -> NONE
 *   v8.1 + AVC420_ENABLED -> AVC420 ; v8.1 without it -> NONE
 *   v10.0 with AVC_DISABLED clear -> AVC444 (v1)
 *   v10.1 (reserved-only capset)  -> NONE (not eligible for the v1 MVP)
